        ...
        return Vue.component("messages-component", {
            ...
            created(){
                this.loadData().then(response => {
                    if(this.response[0].data.messages.welcome[0].messages != undefined){
            ...
            methods: {
                loadData: async function() {
                    try {
                        let results = await Promise.all([this.$store.dispatch('LOAD_PAGE_DATA', {url:this.property.mm_host + "/api/v3/twinpines/messages.json"})]);
                        return results;
                    } catch(e) {
